...
export class TelemetryStreamService {
  ...
  private loadMockDataset(): void {
    this.http.get<BioTelemetryRecord[]>('data/telemetry-mock.json').subscribe({
      ...
      error: (err) =>
        console.error('Failed to load local mock telemetry:', err),
    });
  }
      ...
      .pipe(
        map((responseText: string) => {
          if (
            ...
            throw new Error(
              'Cloudflare worker timeout or upstream Movebank error',
            );
          }
          ...
          return lines
            ...
            .map((lineStr, index) => {
              ...
              const item = headers.reduce(
                (acc: Record<string, string>, h, i) => {
              ...
              const lat = parseFloat(
                item['location_lat'] ?? item['lat'] ?? '0',
              );
              const lng = parseFloat(
                item['location_long'] ?? item['lon'] ?? '0',
              );
              ...
              return {
                ...
                subjectId:
                  ...
                  `sub-${index}`,
                species: this.mapTaxonToSpeciesType(
                  item['taxon_canonical_name'],
                ),
                ...
                coordinates: {
                  ...
                  lng: isNaN(lng) ? 0 : lng,
                  elevationMeters:
                    parseFloat(item['heightAboveEllipsoid'] ?? '0') ||
                    undefined,
                },
                telemetry: {
            ...
            .filter(
              (record) =>
                record.coordinates.lat !== 0 && record.coordinates.lng !== 0,
            );
        }),
        catchError((error) => {
          this.liveError.set(
            error.message || 'Failed to load live telemetry stream',
          );
  ...
  public setStudyId(studyId: string): void {
    ...
    this.fetchLiveTelemetry(studyId).subscribe({
      next: (records) => {
        ...
        if (records.length === 0) {
          this.liveError.set(
            `No valid spatial telemetry points found for Study ID "${studyId}".`,
          );
        }
      },
      error: () => {
